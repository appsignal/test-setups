...
namespace AppSignal\LaravelAppSignal;
...
class AppSignal
{
	...
	public static function initialize(): void
	{
		...
		static::$basePath = static::findLaravelRoot();
	...
	protected static function loadEnv(): void
	{
		...
		if (file_exists(filename: $envPath)) {
			$dotenv = Dotenv::createImmutable(static::$basePath);
			$dotenv->safeLoad();
		}
	}

	protected static function findLaravelRoot(): ?string
	{
		foreach (get_included_files() as $file) {
			if (str_ends_with($file, '/vendor/autoload.php')) {
				$laravelRoot = dirname($file, 2);

				if (
					file_exists($laravelRoot . '/artisan') &&
					file_exists($laravelRoot . '/composer.json') &&
					is_dir($laravelRoot . '/bootstrap')
				) {
					return $laravelRoot;
				}
			}
